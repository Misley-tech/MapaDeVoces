...
parser.add_argument("audioFilesPath",default = "C:/Users/mauro/Documents/Archivoz/Dataset", help="Path to audio files to parse")
...
#ABAJO DE ESTA LINEA TENES QUE PONER EN DEFAULT EL METODO QUE QUERES "tsne" "umap" "pca"
parser.add_argument("--viz-method", default = "umap", help = "tsne, umap or pca. Default: tsne.")
...
if not os.path.exists(PCAfullPath):
    ...
    #print(audioData)
    #audioDataTransformed = utils.doPCA(audioData[:,:-1])
    audioDataTransformed = utils.applyPCA(audioData)
...
if args.viz_method == "tsne":
    ...
    for i, point in enumerate(embedding):
        plt.annotate(str(i), xy=(point[0], point[1]), xytext=(5, 5), textcoords="offset points")
...
for f in audioFilesForExport:
    if os.sep in f:
        clusterName = f[0:f.rfind(os.sep)]
        if not clusterName in clusterNames:
...
if len(clusterNames) > 1:
    ...
    for f in audioFilesForExport:
        if os.sep in f:
            clusterName = f[0:f.rfind(os.sep)]
            if clusterName in clusterNames: #this is redundant but safe
                clusters.append( clusterNames.index( clusterName ) )
            else:
    ...
    jsonSession["sounds"]["clusterNames"] = clusterNames
