...
#    COUNT_FAIL = 20
    ...
    for i in range(0, qtyFilesToProcess):
        try:
            ...
            tmpAudioData = np.copy(tmpAudioData)
...
"""
def applyPCA(df, n_components=2):
    # Separate the features and the folder names
    X = df.drop('FolderName', axis=1).values
    folder_names = df['FolderName'].values
    ...
    pca_df['FolderName'] = folder_names

    # Visualize clusters
    sns.scatterplot(data=pca_df, x='PC1', y='PC2', hue='FolderName', palette='Set1')
...
def getAudioData(audioFiles, features, audioMaxLength=3, qtyFilesToProcess=None, win_length=2048, hop_length=512, sample_rate=22050):
    ...
    for i in range(0, qtyFilesToProcess):
        try:
            ...
            listFolderNames.append(folderName)
    ...
    # Create a DataFrame with audio data and folder names
    df = pd.DataFrame(matrixAudioData)
    df['FolderName'] = listFolderNames
